...
| Term | Short definition | Source group |
|---|---|---|
| A/B | 두 대안을 같은 평가 질문 아래 비교하는 방식. 이 평가 앱에서는 익명화한 Story A와 Story B 중 더 나은 동화를 고르게 한다. | [`project-eval-runtime`](#project-eval-runtime) |
| A3B | 모델 이름에서 토큰 하나를 처리할 때 활성화되는 파라미터가 약 3B라는 표기. 총 파라미터와 상주 메모리 크기는 별도라서 A3B만 보고 3B 모델처럼 적재할 수는 없다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| anytime-valid | 데이터를 계속 들여다보다가 아무 시점에 멈춰도 통계적 보장이 깨지지 않는 성질. 보통의 검정은 중간에 자꾸 확인하며 멈추면 오류율이 부풀지만, 이 성질이 있으면 안전하다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| API | 소프트웨어가 다른 소프트웨어의 기능이나 데이터에 접근할 때 따르는 호출 계약. 사용할 주소, 입력 형식, 인증, 응답과 오류 규칙을 함께 정의한다. | [`browser-storage-and-web-standards`](#browser-storage-and-web-standards) |
| Arena 점수 | 이 시스템의 최종 점수. BT로 추정한 실력값을 체스 레이팅처럼 읽히도록 평균 1000 기준으로 변환한 것. 두 모델의 점수 차가 곧 승률로 환산된다(차이 100이면 약 64%, 400이면 약 91%). | [`project-eval-runtime`](#project-eval-runtime) |
| ARI | Automated Readability Index. 단어당 글자 수와 문장당 단어 수로 영어 텍스트의 미국 학년 수준을 추정하는 공식이다. | [`readability-primary`](#readability-primary) |
...
| CI | Confidence Interval의 약칭. 이 글에서는 추정 불확실성을 나타내는 신뢰구간을 뜻하며 CI/CD의 CI와는 다른 용어다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| CI/CD | 코드 변경을 자동 빌드·테스트하는 지속적 통합과, 검증된 변경을 배포하는 지속적 전달 또는 배포 절차를 묶어 부르는 말이다. | [`application-and-delivery`](#application-and-delivery) |
| Cloud Run | 컨테이너 이미지를 요청 기반으로 실행하는 Google Cloud의 관리형 플랫폼. 인스턴스 수를 자동 조절할 수 있지만 시작 지연과 최소 인스턴스 설정은 별도로 고려한다. | [`application-and-delivery`](#application-and-delivery) |
| CMI | Custom Model Import. 지원되는 구조의 사용자 모델 가중치를 Amazon Bedrock으로 가져와 관리형 추론에 사용하는 기능이다. | [`aws-bedrock-and-cost`](#aws-bedrock-and-cost) |
| CMU | Custom Model Unit. Bedrock이 임포트한 모델 사본 하나를 서빙하는 데 필요한 하드웨어 사용량의 추상 단위. 모델을 임포트하면 몇 CMU가 필요한지 확정되고, 과금은 CMU 수 × 분 단위로 계산된다. | [`aws-bedrock-and-cost`](#aws-bedrock-and-cost) |
| Cohen's κ | 두 평가자의 범주형 판정 일치도에서 주변분포로 기대되는 우연 일치를 보정한 계수. 1은 완전 일치, 0은 기대 일치 수준이며 음수도 가능하다. | [`statistics-and-ranking`](#statistics-and-ranking) |
...
| DLQ | Dead Letter Queue. 반복 실패한 작업을 본 큐에서 빼내 격리 보관하는 별도 큐. 문제 있는 작업 하나가 큐 전체를 막는 것을 방지하고, 실패분을 나중에 조사해 재처리할 수 있게 한다. | [`application-and-delivery`](#application-and-delivery) |
...
| FastAPI | Python 타입 힌트를 이용해 요청 검증과 OpenAPI 문서를 생성하는 ASGI 웹 프레임워크. 이 프로젝트에서는 Telegram webhook을 받는 HTTP 서버에 사용했다. | [`application-and-delivery`](#application-and-delivery) |
| Fisher-Yates | 배열의 뒤쪽부터 아직 확정되지 않은 위치 하나를 균등하게 골라 교환하는 셔플 알고리즘. 균등한 난수 생성기를 쓰면 모든 순열이 같은 확률로 나오며, 재현성은 난수 생성기와 구현까지 같아야 한다. | [`statistics-and-ranking`](#statistics-and-ranking) |
...
| HTTP | 클라이언트와 서버가 요청과 응답을 주고받는 웹 전송 규약. 메서드, 상태 코드, 헤더와 본문 형식을 정의하며 TLS를 사용한 암호화 연결은 HTTPS라고 부른다. | [`browser-storage-and-web-standards`](#browser-storage-and-web-standards) |
| IFEval | 검증 가능한 지시를 포함한 프롬프트와 규칙 기반 채점기로 구성된 지시 준수 벤치마크. 사람이나 LLM 심판 대신 프로그램으로 조건 충족 여부를 확인한다. | [`llm-evaluation-primary`](#llm-evaluation-primary) |
| IndexedDB | 브라우저가 출처별로 제공하는 비동기 객체 데이터베이스. localStorage보다 큰 구조화 데이터를 다루지만 저장 한도와 삭제 정책은 브라우저·디스크 여유·사용자 설정에 따라 달라진다. | [`browser-storage-and-web-standards`](#browser-storage-and-web-standards) |
| InfoBench | 지시를 세부 요구사항으로 분해해 각각의 충족 여부를 따로 채점하는 지시 준수 벤치마크. 하나의 응답에서 어떤 요구가 지켜지고 어떤 것이 무시됐는지를 분리해 보여준다. | [`llm-evaluation-primary`](#llm-evaluation-primary) |
| InvokeModel | Bedrock의 저수준 호출 API. 모델별 네이티브 요청 형식을 그대로 보낸다. 통일 인터페이스인 Converse를 지원하지 않는 모델은 이쪽으로 호출해야 한다. | [`aws-bedrock-and-cost`](#aws-bedrock-and-cost) |
| ITPM | Input Tokens Per Minute. 분당 입력 토큰 한도. 프롬프트가 길고 호출이 잦은 워크로드에서는 출력보다 입력 한도가 먼저 바닥나 병목이 되기도 한다. | [`openrouter-runtime`](#openrouter-runtime) |
| JSON | 문자열·숫자·불리언·배열·객체·null을 표현하는 텍스트 데이터 형식. 주석과 trailing comma는 표준 JSON에 포함되지 않는다. | [`browser-storage-and-web-standards`](#browser-storage-and-web-standards) |
| jury | 서로 다른 회사 계열의 심판 여러 명을 함께 쓰는 구성. 심판마다 취향과 편향이 다른데, 계열을 섞으면 한 심판의 성향이 순위를 좌우하는 걸 줄일 수 있다. | [`project-eval-runtime`](#project-eval-runtime) |
| K-팩터 | Elo에서 경기 하나가 점수를 최대 몇 점까지 움직일 수 있는지 정하는 민감도 상수. 크면 최근 경기에 민감해지고 작으면 점수가 천천히 변한다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| Krippendorff α | 평가자 수가 일정하지 않거나 일부 판정이 비어 있어도 계산할 수 있는 신뢰도 계수. 자료 척도에 맞는 불일치 함수를 쓰며 1은 완전 일치, 0은 우연 기대 수준이다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| KV 캐시 | Transformer가 이미 처리한 토큰의 attention key와 value를 저장해 다음 토큰 생성 때 다시 계산하지 않게 하는 메모리. 동시 요청과 문맥 길이가 늘면 필요한 메모리도 커진다. | [`llm-core-and-inference`](#llm-core-and-inference) |
| L4 | NVIDIA의 추론용 GPU로 메모리가 24GB다. 20GB 안팎의 양자화 가중치도 KV 캐시와 런타임 버퍼를 더하면 여유가 작아 긴 컨텍스트나 높은 동시성에 불리하다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| L40S | NVIDIA의 서빙·그래픽 겸용 GPU로 VRAM 48GB. 모델 가중치와 KV 캐시가 VRAM에 다 들어가야 서빙이 성립하므로, 24GB(L4)냐 48GB(L40S)냐가 올릴 수 있는 모델의 상한을 가른다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| Lexile | 영어 독자의 읽기 능력과 텍스트 난이도를 같은 척도에 표시하는 상용 지표. 텍스트 측정은 어휘의 빈도와 문장 길이 같은 특성을 사용하며, 이 프로젝트의 가독성 공식 합성값은 공인 Lexile이 아니다. | [`readability-primary`](#readability-primary) |
| linear attention | 어텐션 계산을 시퀀스 길이에 비례(선형)하도록 근사하는 방식. 긴 문맥에서 메모리와 속도가 유리해 최신 하이브리드 모델이 다수 층에 채택하지만, 표준 어텐션과 구현이 달라 학습·서빙 도구의 지원 여부를 따로 확인해야 한다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| llama.cpp | C/C++로 구현된 오픈소스 LLM 추론 도구. CPU와 여러 GPU 백엔드에서 GGUF 모델을 실행하며 애플 실리콘의 Metal도 지원한다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| LLM | Large Language Model. 많은 텍스트에서 토큰의 조건부 분포를 학습해 문장을 생성하거나 분류·요약·추론 작업을 수행하는 언어 모델을 뜻한다. | [`llm-core-and-inference`](#llm-core-and-inference) |
| LLM-as-a-Judge | 사람 대신 LLM에게 후보 응답을 비교하거나 점수화하게 하는 평가 방식. 빠르게 확장할 수 있지만 위치·길이·자기 선호 같은 편향을 별도로 통제하고 사람 기준과 대조해야 한다. | [`llm-evaluation-primary`](#llm-evaluation-primary) |
| LMArena | Chatbot Arena의 현재 이름. 익명 A/B 대결에 사람들이 투표한 선호를 Bradley-Terry로 집계해 공개하는 품질 리더보드다. | [`llm-evaluation-primary`](#llm-evaluation-primary) |
| localStorage | 브라우저가 출처별로 제공하는 동기식 문자열 키-값 저장소. Web Storage 한도는 브라우저에 따라 다르고 한도를 넘으면 QuotaExceededError가 날 수 있어 큰 체크포인트 저장에는 맞지 않는다. | [`browser-storage-and-web-standards`](#browser-storage-and-web-standards) |
| LoRA | 원본 가중치는 얼려 두고 곁에 붙인 작은 저랭크 행렬(어댑터)만 학습하는 파인튜닝 기법. 학습 대상이 전체의 1% 미만이라 메모리와 시간이 크게 줄고, 어댑터만 따로 저장·교체할 수 있다. | [`post-training-and-model-files`](#post-training-and-model-files) |
| LoRA rank | LoRA가 원래 가중치 변화를 두 작은 행렬로 표현할 때 쓰는 내부 차원. 높이면 표현력과 학습 파라미터가 늘고 낮추면 메모리와 파일 크기가 줄지만 적정값은 데이터와 대상 층에 따라 달라진다. | [`post-training-and-model-files`](#post-training-and-model-files) |
| MAE | 예측값과 실제값 차이의 절댓값을 평균한 지표. 작을수록 평균 오차가 작으며, 큰 오차를 제곱하는 RMSE와 달리 오차 크기에 선형으로 반응한다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| Match Rate | 목표 난이도의 허용 범위(±밴드) 안에 들어온 비율. 절대 눈금이 틀린 측정기로 재면 실력과 무관하게 낮게 나올 수 있다. | [`project-eval-runtime`](#project-eval-runtime) |
| Metal | Apple 플랫폼의 그래픽·병렬 연산 API. MLX는 Apple Silicon GPU에서 계산할 때 Metal 기반 실행 경로를 사용한다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| MLE | Maximum Likelihood Estimation의 약칭. 관측된 데이터의 우도를 가장 크게 만드는 파라미터를 선택하는 최우도 추정이다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| MLX | Apple이 애플 실리콘용으로 만든 배열·머신러닝 프레임워크. CPU와 GPU가 공유하는 통합 메모리 모델을 사용하며 추론과 학습을 지원한다. | [`post-training-and-model-files`](#post-training-and-model-files) |
| MM 알고리즘 | 어려운 목적함수 대신 현재 점에서 맞닿는 대리함수를 반복 최적화하는 Majorization–Minimization 계열 방법. Hunter(2004)의 Bradley–Terry 갱신은 매 반복 우도가 감소하지 않도록 구성된다. | [`statistics-and-ranking`](#statistics-and-ranking) |
...
| Qwen3.6-35B-A3B | 총 파라미터 약 35B 중 토큰마다 약 3B를 선택하는 MoE 구조. 전체 가중치 메모리는 필요하며 실제 속도는 라우팅·메모리 대역폭·커널·캐시에 따라 달라진다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| racing | 후보들을 경주시키다 신뢰구간이 확실히 갈라진 시점에 열등한 쪽을 탈락시키는 순차 기법의 통칭. 1993년 Hoeffding Races에서 이름이 왔다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| retire2 | 이 프로젝트의 은퇴 스케줄러. 배치(5권)마다 BT와 CI를 갱신하고, CI 분리가 2연속 확인된 모델만 은퇴시킨다. 한 번의 우연으로 탈락시키지 않는 확인 절차가 이름의 유래다. | [`project-eval-runtime`](#project-eval-runtime) |
...
| t-검정 | 평균 차이를 표준오차와 비교해 귀무가설 아래에서 관측값이 얼마나 이례적인지 평가하는 검정. 유의성은 t값 하나가 아니라 자유도, 검정 방향, p값 또는 임계값과 함께 판단한다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| temperature | 다음 토큰 확률분포의 평탄함을 조절하는 값. 낮을수록 높은 확률의 토큰에 집중하지만 temperature 0도 제공자·모델·실행 환경에 따라 완전한 재현성을 보장하지 않는다. | [`llm-core-and-inference`](#llm-core-and-inference) |
| top-k | 상위 k개 집합. 여기서는 1위와 신뢰구간이 겹쳐 아직 우열을 다투는 상위 클러스터를 뜻하며, 은퇴 판정의 기준선이 된다. | [`project-eval-runtime`](#project-eval-runtime) |
| TPM | Tokens Per Minute. 1분 동안 허용하거나 처리한 토큰 수. 입력·출력을 합치거나 따로 제한하는지는 공급자 정의를 확인해야 한다. | [`openrouter-runtime`](#openrouter-runtime) |
| transformers | Hugging Face가 제공하는 모델·토크나이저 로딩과 학습 라이브러리. 새 아키텍처는 이를 구현한 라이브러리 버전이 있어야 로드할 수 있으므로 서빙 환경의 지원 버전을 확인해야 한다. | [`post-training-and-model-files`](#post-training-and-model-files) |
| TRL | Hugging Face의 post-training 라이브러리. SFT, DPO, PPO 등 여러 학습·정렬 트레이너를 제공한다. | [`post-training-and-model-files`](#post-training-and-model-files) |
| TTFT | Time to First Token. 요청을 보낸 시점부터 스트리밍 응답의 첫 토큰을 받을 때까지 걸린 시간으로, 출력이 시작되는 체감 대기 시간을 나타낸다. | [`llm-core-and-inference`](#llm-core-and-inference) |
| TTL | Time To Live. 데이터에 걸어 두는 유효 시간으로, 지나면 자동 삭제된다. 진행률처럼 잠깐만 의미 있는 값을 별도 청소 코드 없이 관리할 수 있다. | [`browser-storage-and-web-standards`](#browser-storage-and-web-standards) |
...
| 모델 리비전 | 같은 모델 저장소 안의 특정 커밋이나 버전 식별자. 학습·변환·서빙에서 같은 리비전을 고정해야 조용한 파일 변경으로 결과가 달라지는 일을 막을 수 있다. | [`post-training-and-model-files`](#post-training-and-model-files) |
| 모델 체크포인트 | 특정 학습 시점의 가중치와 설정을 함께 저장한 묶음. 같은 이름의 모델도 리비전이나 후처리 방식이 다르면 별도 체크포인트로 취급해야 한다. | [`model-formats-and-serving`](#model-formats-and-serving) |
| 모델 파라미터 | 학습 과정에서 조정되는 모델의 수치 값. 파라미터 수는 모델 규모를 나타내지만 실제 메모리와 속도는 데이터 형식·구조·실행 방식에도 좌우된다. | [`model-formats-and-serving`](#model-formats-and-serving) |
...
| 신뢰구간 | 같은 절차로 표본을 반복 수집할 때 정해진 비율만큼 모수를 포함하도록 만든 구간. 두 개의 95% 신뢰구간이 겹치는지만으로 차이의 유의성을 판정할 수는 없다. | [`statistics-and-ranking`](#statistics-and-ranking) |
| 실청구액 | 오픈라우터가 모든 응답의 usage 객체에 실어주는 실제 청구 금액(usage.cost). 토큰 수에 단가를 곱해 추정하는 것이 아니라 계정에서 실제로 빠져나간 크레딧이다. | [`openrouter-runtime`](#openrouter-runtime) |
...
| 어댑터 | 동결한 베이스 모델 옆에 붙여 학습하는 작은 추가 가중치 묶음. 파일이 작아도 층 이름과 배열 모양이 실행 프레임워크의 형식과 맞지 않으면 그대로 옮겨 쓸 수 없다. | [`post-training-and-model-files`](#post-training-and-model-files) |
...
| 컨텍스트 길이 | 한 요청에서 모델이 한꺼번에 참고할 수 있는 입력과 출력 토큰 범위. 길게 잡을수록 KV 캐시와 임시 메모리가 늘어 같은 GPU에서 처리할 동시 요청 수가 줄 수 있다. | [`model-formats-and-serving`](#model-formats-and-serving) |
...
### aws-bedrock-and-cost

Terms: `CMI`, `CMU`, `Converse`, `EC2`, `InvokeModel`, `SageMaker Async Inference`, `관리형 API`, `손익분기`, `스팟`, `온디맨드`
...
Terms: `데이터 누수`, `선호쌍`, `홀드아웃`

- <https://scikit-learn.org/stable/common_pitfalls.html#data-leakage>
- <https://scikit-learn.org/stable/modules/cross_validation.html>
- <https://arxiv.org/abs/2305.18290>
- `project:C0mput33/little-bard/eval/`

### external-benchmark-services

Terms: `Artificial Analysis`
...
Terms: `Chatbot Arena`, `CoT`, `EQ-Bench`, `G-Eval`, `GSM8K`, `IFEval`, `InfoBench`, `LLM-as-a-Judge`, `LMArena`, `MMLU`, `루브릭`
...
- `project:C0mput33/little-bard/eval/config/models.yaml`
...
Terms: `DPO`, `LoRA`, `LoRA rank`, `MLX`, `PEFT`, `QLoRA`, `SFT`, `TRL`, `그래디언트`, `그래디언트 누적`, `그래디언트 체크포인팅`, `모델 리비전`, `어댑터`, `옵티마이저 상태`, `파인튜닝`, `safetensors`, `transformers`
...
- `project:C0mput33/little-bard/eval/config/rubric.yaml`
