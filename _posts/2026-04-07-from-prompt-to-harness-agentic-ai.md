...
Chain-of-Thought[^1] 이 나왔고, "Let's think step by step."이 <span class="term" data-tip="초등학교 수준 수학 서술형 문제 약 8,500개로 다단계 추론을 재는 벤치마크. 공개된 지 오래돼 학습 데이터 오염 논쟁의 대표 사례가 됐다.">GSM8K</span> 벤치마크에서 17.9% → 58.1%로 끌어올렸다. 엄밀함의 위치가 모델 자체에서 **프롬프트**로 이동하기 시작한 시기다.
...
현재 병목은 system prompt의 문구보다 컨텍스트 선택과 실행 실패 처리에 있다. 모든 대화 히스토리를 보내는 방식을 관련 정보 선별로 바꾸고, 도구별 실패를 격리한 뒤, 현재 사용량에서 멀티 에이전트의 추가 지연과 비용을 측정해야 한다. 그 검증 전에는 이 시스템을 Stage 3라고 부르지 않기로 했다.
