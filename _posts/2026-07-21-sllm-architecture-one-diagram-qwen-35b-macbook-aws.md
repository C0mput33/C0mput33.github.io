...
## A-1. Little Bard의 로컬 평가는 운영 서빙과 분리한다

여기에는 서로 다른 두 연결이 있다. Little Bard 평가 앱이 학습 전후 모델을 비교하는 **로컬 평가 연결**과, Bookkiki 사용자의 동화 요청을 처리하는 **제품 서빙 연결**이다. 앞쪽은 실험 재현성이 목적이고 뒤쪽은 인증·내구성·확장성이 목적이다. 같은 모델을 쓰더라도 한 엔드포인트로 뭉치지 않는다.

현재 Little Bard <span class="term" data-tip="웹 문서의 구조와 의미를 요소로 표시하는 마크업 언어. CSS는 표현을, JavaScript는 동작을 주로 담당한다.">HTML</span>에는 이미 `ollama/<tag>` 모델을 `http://localhost:11434/v1/chat/completions`로 보내는 로컬 경로가 있다. Python 파이프라인에도 같은 <span class="term" data-tip="특정 공급자나 실행 환경의 호출 방식을 공통 인터페이스 뒤에 감추는 구성요소. 평가 로직은 그대로 두고 OpenRouter·Ollama·MLX 같은 실행 경로만 교체할 수 있게 한다.">Provider Adapter</span>가 있다.[^localeval]

새 Qwen3.6 MLX 경로는 평가 로직을 하나 더 만드는 작업이 아니다. 기존 공급자 경계를 일반화해 MLX 서버도 같은 요청·응답·오류 계약으로 연결하는 작업이다.

2026년 7월 23일 확인한 로컬 스모크는 여기까지다.

| 항목 | 확인한 상태 | 아직 하지 않은 일 |
|---|---|---|
| 모델 | `mlx-community/Qwen3.6-35B-A3B-4bit` 고정 리비전 다운로드·무결성 검사 | LoRA 학습과 어댑터 재적재 |
| 실행 | M5 Pro 48GB에서 CLI와 <span class="term" data-tip="Python 함수와 모델 데모를 웹 UI로 감싸고, 연결된 이벤트 함수의 호출 문서를 자동 생성하는 오픈소스 도구. 자동 API 노출은 개발 편의 기능이며 인증이나 네트워크 보안 자체를 대신하지 않는다.">Gradio</span> UI 생성 성공 | Little Bard HTML과의 실제 연결 |
| 로컬 API | Gradio가 `generate_story` 함수의 개발용 API 문서를 자동 생성 | 평가용 OpenAI 호환 계약·모델 manifest |
| 평가 | 기존 <span class="term" data-tip="오픈 웨이트 모델을 로컬 장비에서 내려받아 실행하고 API로 호출하게 해주는 도구. 외부 모델 API의 호출 요금은 없지만 장비·전력·운영 비용은 별도다.">Ollama</span> 로컬 공급자 코드가 존재 | Qwen base와 LoRA를 별도 ID로 등록한 블라인드 비교 |

Gradio는 이벤트 함수 이름으로 API 엔드포인트와 클라이언트 예제를 자동 생성한다. `api_visibility="private"`로 문서와 Gradio 클라이언트 노출을 줄일 수 있지만, 공식 문서도 직접 <span class="term" data-tip="클라이언트와 서버가 요청과 응답을 주고받는 웹 전송 규약. 메서드, 상태 코드, 헤더와 본문 형식을 정의하며 TLS를 사용한 암호화 연결은 HTTPS라고 부른다.">HTTP</span> 접근을 막는 보안 기능은 아니라고 명시한다.[^gradioapi]

따라서 현재 `127.0.0.1:7860` Gradio는 사람이 모델 적재와 출력을 확인하는 개발 스모크로만 남긴다. Little Bard의 목표 연결은 아래처럼 <span class="term" data-tip="OpenAI Chat Completions와 비슷한 URL·JSON 요청·응답 형식을 구현해 같은 클라이언트 코드로 다른 모델 서버를 호출할 수 있게 한 인터페이스. 세부 지원 필드는 서버마다 달라 별도 확인이 필요하다.">OpenAI 호환 API</span>로 수렴시킨다. vLLM은 `/v1/chat/completions`와 `/v1/models`를 제공하고, LoRA 어댑터를 요청의 `model` 값으로 선택할 수 있다.[^vllmopenai]

![Little Bard 로컬 평가 브리지와 Bookkiki 운영 경계](/assets/img/posts/2026-07/little-bard-local-eval-bridge.svg)
_그림 2. 로컬 평가는 base와 LoRA를 같은 조건으로 재는 경로다. 제품 요청은 Gradio를 거치지 않고 비동기 Job API와 운영용 모델 서버를 사용한다._

### 평가 앱에 넣을 공급자 계약

평가 HTML의 모델 표시는 서버 주소와 분리한다. 화면에는 논리 모델 ID를 보여 주고, 공급자 설정이 실제 주소를 관리한다.

| 논리 모델 ID | 실제 의미 | 같은 것으로 취급하면 안 되는 것 |
|---|---|---|
| `local-qwen-base` | 고정 리비전의 MLX 4bit 베이스 | OpenRouter의 호스팅 Qwen3.6 |
| `local-qwen-childlit-lora-v1` | 같은 베이스 + 검증된 동화 LoRA 체크섬 | 학습 중간 어댑터, 다른 베이스 리비전 |

목표 요청은 아래 형태다. 이것은 현재 Gradio에 이미 구현됐다는 뜻이 아니라, Little Bard의 기존 Ollama 경로와 이후 vLLM 경로가 공유할 계약이다.

```json
{
  "model": "local-qwen-base",
  "messages": [
    {"role": "system", "content": "<childlit-v3 system prompt>"},
    {"role": "user", "content": "<holdout prompt>"}
  ],
  "temperature": 0.7,
  "max_tokens": 736,
  "stream": false
...
호출 전에 `/v1/models` 또는 읽기 전용 `/metadata`에서 현재 적재된 모델 manifest를 확인한다. 요청한 논리 ID와 서버의 `resolved_model`, 베이스 <span class="term" data-tip="같은 모델 저장소 안의 특정 커밋이나 버전 식별자. 학습·변환·서빙에서 같은 리비전을 고정해야 조용한 파일 변경으로 결과가 달라지는 일을 막을 수 있다.">모델 리비전</span>, 토크나이저 리비전, 어댑터 버전·체크섬 중 하나라도 다르면 생성 전에 중단한다. URL이 응답했다는 사실만으로 올바른 모델이 올라왔다고 보지 않는다.

M5 Pro 48GB에서는 20GB대 베이스 두 벌을 항상 동시에 띄우는 구성을 기본으로 삼지 않는다. 먼저 base 서버로 holdout 프롬프트 전체를 생성해 체크포인트를 저장하고, 프로세스를 종료한 뒤 LoRA 어댑터를 붙인 서버로 같은 프롬프트를 생성한다. 동시 적재나 요청별 hot swap은 해당 MLX 런타임에서 메모리·상태 격리를 검증한 뒤에만 사용한다.

### 결과에 무엇을 남길 것인가

동화 본문만 저장하면 학습 전후 비교를 재현할 수 없다. 각 호출에는 최소한 다음 값을 함께 남긴다.
...
이 기록은 기존 <span class="term" data-tip="어떤 정책 버전의 system·user 메시지가 실제 API에 전달됐는지 남기는 출처 기록. 결과와 함께 저장해야 과거 프롬프트를 현재 프롬프트로 잘못 재구성하지 않는다.">프롬프트 provenance</span>에 로컬 런타임 정보를 더한 것이다. 로컬 호출은 외부 토큰 과금이 0이어도 비용 필드를 지우지 않는다. `api_cost_usd=0`과 함께 실행 시간·전력·장비 점유를 별도 관측해야 “비용이 없다”와 “API 청구가 없다”를 구분할 수 있다.

### base와 LoRA를 같은 평가에 넣는 규칙

1. 학습에 사용하지 않은 holdout 프롬프트를 고정한다.
2. base와 LoRA에 같은 `childlit-v3`, temperature, token 상한, thinking 설정을 보낸다.
3. 각각을 별도 model ID와 별도 생성 체크포인트로 저장한다.
4. 생성 정책·공급자 계약·베이스 리비전·어댑터 버전이 다른 체크포인트는 증분 이어하기로 섞지 않는다.
5. 로컬 연결 실패, 모델 불일치, 빈 출력, 잘린 출력은 `tie`가 아니라 생성 `invalid`로 제외한다.
6. 유효 동화만 모델 이름을 숨겨 기존 양방향 <span class="term" data-tip="두 대안을 같은 평가 질문 아래 비교하는 방식. 이 평가 앱에서는 익명화한 Story A와 Story B 중 더 나은 동화를 고르게 한다.">A/B</span>와 다계열 심판에 전달한다.
7. <span class="term" data-tip="Bradley–Terry 모델의 약칭. 두 후보의 상대적 실력으로 맞대결 승률을 설명하고 전체 pairwise 결과에서 실력값을 추정한다.">BT</span>뿐 아니라 유효 응답률, 길이 준수율, 지연, 메모리와 총원가를 함께 보고 LoRA 채택 여부를 결정한다.

로컬 Qwen으로 생성하는 것과 심판까지 완전히 오프라인으로 돌리는 것은 다른 주장이다. 생성 후보만 로컬이고 심판이 OpenRouter라면 심판 호출에는 인터넷과 API 키가 필요하다. 완전 오프라인 평가를 하려면 생성 모델과 계열이 겹치지 않는 로컬 심판을 따로 준비하고, 자기 계열 제외 뒤에도 모든 pair에 유효 심판이 남는지 사전 검사해야 한다.

### 브라우저와 보안 경계

Little Bard HTML을 `file://`로 직접 열면 브라우저가 문서를 불투명한 origin으로 취급해 로컬 API 응답을 읽지 못할 수 있다. MDN도 로컬 시험에는 파일을 직접 여는 대신 HTTP 개발 서버를 사용하라고 안내한다.[^browserorigin] 따라서 평가 앱은 `http://127.0.0.1:<eval-port>`로 열고, 로컬 모델 서버는 해당 origin만 <span class="term" data-tip="Cross-Origin Resource Sharing. 브라우저가 현재 페이지와 다른 scheme·host·port의 서버 응답을 읽어도 되는지 서버 헤더로 허용하는 규칙. 허용 origin을 넓게 잡으면 의도하지 않은 웹페이지도 로컬 API를 호출할 수 있다.">CORS</span> 허용 목록에 둔다.

`127.0.0.1` 바인딩, 정확한 origin 허용, 외부 공유 링크 비활성화를 개발 기본값으로 둔다. 향후 다른 기기나 Bookkiki 서버에서 접근해야 하면 Gradio를 그대로 공개하지 않고 인증·rate limit·로그 정책을 갖춘 별도 서비스 경계를 만든다. 숨겨진 API 문서나 `api_visibility` 설정만으로 인증을 대신하지 않는다.

### 구현 순서

1. 기존 Ollama 전용 <span class="term" data-tip="API에서 모델을 지정하는 고유 식별 문자열(예: openai/gpt-5.2). 오타가 나면 호출 시점에야 404로 드러난다.">slug</span> 분기를 `provider + base_url + model` 설정으로 일반화한다.
2. 현재 Gradio 스모크와 별도로 OpenAI 호환 로컬 브리지를 만들고 `/v1/models`·health·manifest를 제공한다.
3. `local-qwen-base` 한 모델로 프롬프트 1개 생성·메타 저장·invalid 처리를 확인한다.
4. base 전체 holdout 출력을 저장한 뒤 LoRA 서버를 재시작하고 `local-qwen-childlit-lora-v1`을 같은 조건으로 실행한다.
5. 결과 화면에 요청·해결 모델, 베이스 리비전, 어댑터 버전, 런타임, 토큰·지연·메모리를 노출한다.
6. 체크포인트 호환성 검사와 사람 확인용 소규모 smoke를 통과한 뒤에만 전체 BT 비교를 시작한다.
7. 이 로컬 평가가 통과한 모델만 Bookkiki 비동기 워커의 운영 Provider Adapter 후보로 넘긴다.
...
[^localeval]: Little Bard의 [HTML 로컬 라우팅](https://github.com/C0mput33/little-bard/blob/main/eval/app/index.html)과 [Python LocalProvider](https://github.com/C0mput33/little-bard/blob/main/eval/providers/local.py) — `ollama/<tag>`를 로컬 `/v1/chat/completions`로 보내고 requested/resolved model, usage, 지연과 오류를 공통 메타로 반환한다. Qwen3.6 MLX 연결은 이 계약을 일반화할 예정이며 아직 구현 완료가 아니다. 2026-07-23 확인.
[^gradioapi]: Gradio, [View API Page](https://www.gradio.app/guides/view-api-page)와 [Gradio 6 migration guide](https://www.gradio.app/guides/gradio-6-migration-guide) — 이벤트 함수 이름에서 API 문서를 자동 생성하고 `public`·`undocumented`·`private` 공개 수준을 제공한다. `private`도 직접 HTTP 접근을 막는 보안 장치가 아니므로 네트워크 바인딩과 인증을 별도로 적용해야 한다. 2026-07-23 확인.
[^vllmopenai]: vLLM, [OpenAI-Compatible Server](https://docs.vllm.ai/en/latest/serving/openai_compatible_server/)와 [LoRA Adapters](https://docs.vllm.ai/en/latest/features/lora/) — `/v1/chat/completions`, `/v1/models`와 LoRA 서빙을 제공하며 요청의 `model` 값으로 등록한 어댑터를 선택할 수 있다. 런타임 동적 LoRA 적재는 공식 문서도 신뢰된 로컬 개발 환경에 한정하라고 경고한다. 2026-07-23 확인.
[^browserorigin]: MDN, [Origin](https://developer.mozilla.org/en-US/docs/Glossary/Origin), [CORS request not HTTP](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CORS/Errors/CORSRequestNotHttp), [CORS allow-origin 오류](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CORS/Errors/CORSMissingAllowOrigin) — 현대 브라우저는 `file:` 문서를 대개 불투명 origin으로 취급한다. 로컬 시험은 HTTP 서버로 열고 서버가 필요한 origin만 허용해야 한다. 2026-07-23 확인.
