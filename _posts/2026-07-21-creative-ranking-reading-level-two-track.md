...
> **구현 상태 — 2026-07-22.** 아래 계약은 Little Bard의 `feat/eval-prompt-provenance` 브랜치 `acf70cf`에서 Python·브라우저 동등성 테스트를 통과했다. 기본 `main`은 아직 `aligned-v2`인 `10d97cc`다. 새 정책의 유료 OpenRouter 스모크와 사람 골든셋 교정은 진행하지 않았다. 따라서 이 글은 새 유료 순위 결과가 아니라, 검증 중인 설계와 구현의 근거다.
{: .prompt-warning }
