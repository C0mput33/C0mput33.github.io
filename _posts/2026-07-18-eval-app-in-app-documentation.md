...
**2026-07-20~22 구현 정합성 감사.** pairwise는 순위를 만드는 형식이고, 무엇을 비교할지는 여전히 평가 기준과 생성 맥락이 필요하다. 첫 감사 전 Python 심판은 설정의 축 ID만 받았고 브라우저는 별도 7축 프롬프트를 썼다. `aligned-v2`에서 세부 criteria·앵커와 목표 Lexile까지 맞췄지만, 다시 보니 창작 품질과 수치 난이도 제어가 한 순위에 섞였다. 후속 `childlit-v3`는 Python·브라우저가 같은 10축 기준을 쓰되 창작 본선과 심판에서 수치 Lexile을 제거했다. 본선은 원 요청·독서 방식·연령·실측 단어 수만 보고, <span class="term" data-tip="Flesch–Kincaid Grade Level. 문장당 단어 수와 단어당 음절 수로 영어 텍스트의 미국 학년 수준을 추정한다.">FKGL</span>·<span class="term" data-tip="글자 수와 문장 수를 사용해 영어 텍스트의 미국 학년 수준을 추정하는 가독성 지수. 음절 수를 직접 세지 않는다는 점이 FKGL과 다르다.">Coleman–Liau</span>·<span class="term" data-tip="Automated Readability Index. 단어당 글자 수와 문장당 단어 수로 영어 텍스트의 미국 학년 수준을 추정하는 공식이다.">ARI</span>와 목표 Lexile은 기본 OFF인 독립 난이도 프로브에만 남겼다. 자동 근사값은 순위·은퇴에 영향을 주지 않는다. 설계 근거와 현재 브랜치 상태는 [프롬프트 재설계 글](/posts/creative-ranking-reading-level-two-track/)에 정리했다.
...
- **운영 카드(⑦, ⑨~⑫)**: 로드맵, 프롬프트와 코드의 책임 분담, 루브릭 축별 비중(안전 0.16·재미 0.15 등 — 아래 정정 1건의 주인공), 저장·재개 구조.
