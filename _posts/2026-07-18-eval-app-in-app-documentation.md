...
**불확실성은 부트스트랩 CI로.** BT 실력값의 오차를 주는 닫힌 수식이 없어, (쌍, 심판) 단위 클러스터 재표집 1,000회로 95% 구간을 만든다.[^boot] 반복 60회였던 초기 구현은 백분위 끝값의 순서통계량 불안정 때문에 구간을 과소평가했고(5편), 재표집 단위가 판정 개별이면 같은 쌍 안의 상관을 무시해 역시 좁아진다 — 클러스터 단위 재표집이 표준 처방인 이유다.
