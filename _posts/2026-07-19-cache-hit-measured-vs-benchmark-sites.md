...
여러 사이트를 뒤졌다. Artificial Analysis는 속도·지연·지능 지수를 다루지만 캐시는 없다.[^aa] LMArena는 선호 투표다. **캐시 히트율을 공개하는 곳은 오픈라우터의 Effective Pricing 섹션이 유일했다** — 각 모델 페이지에 공급자별로 "지난 30일 전체 고객 트래픽의 가중 평균 캐시 히트율"과, 캐싱 반영 후 실효 입력 단가를 보여준다.
