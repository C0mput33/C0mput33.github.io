...
description: >-
  ...
  공식 문서의 예측이 실측에서 3건 모두 적중했고, 오픈라우터가 공개하는 공급자별 캐시 히트율과
  대조했다. 7월 19일 캡처와 7월 23일 현재값이 달라진 이유와 각 지표의 시간창도 함께 기록한다.
...
> **캡처와 현재 화면이 다른 이유**
> 이 글의 이미지 7장은 모두 **2026-07-19에 저장한 스냅샷**이다. 고정 단가표를 찍은 자료가 아니다. OpenRouter의 Effective Pricing은 최근 30일 롤링 평균이고, 같은 표의 Token share는 최근 1일 비중이다. 공급자 라우팅 통계는 최근 5분 창으로 갱신되며, 모델 페이지의 성능 그래프는 선택한 기간(이 글의 캡처는 1주)을 보여준다. Artificial Analysis의 첫 화면도 최근 72시간 측정의 중앙값이다.[^orpricing][^orperf][^aa] 따라서 같은 URL을 오늘 열면 숫자와 공급자 순서가 달라지는 것이 정상이다. 과거 이미지는 당시 관측 증거로 보존하고, **2026-07-23 재확인값**을 아래 표에 별도로 병기했다.
...
| Token share (1d) | 최근 1일 동안 해당 공급자가 처리한 전체 토큰의 몫 | 30일 가격 평균과 시간창이 다르다. 가중 평균의 최근 공급자 구성을 설명하는 참고치이며 내 요청의 라우팅 확률은 아니다 |
...
_2026-07-19 보관 스냅샷. Opus 4.8의 당시 가중 평균 입력은 $1.67/M이고 공급자별 캐시힛은 Anthropic 81.5%·Bedrock(US) 78.9%·Azure 0.0%였다. `Token share`가 달라 공급자 행을 단순평균하면 안 된다. 원본 위치: [OpenRouter Opus 4.8 Pricing](https://openrouter.ai/anthropic/claude-opus-4.8/pricing). 현재 화면은 아래 재확인 표와 같이 달라질 수 있다._

실측과 나란히 놓으면 이렇다.

| 모델 | 실측 캐시힛(우리 워크로드) | 오픈라우터 공개값(2026-07-19 캡처) | 해석 |
|---|---|---|---|
...
_2026-07-19 보관 스냅샷. GPT-5.2의 당시 가중 평균 입력은 $1.47/M, OpenAI 행의 캐시힛은 14.2%·token share는 91.5%, Azure 행은 55.9%·8.5%였다. 원본 위치: [OpenRouter GPT-5.2 Pricing](https://openrouter.ai/openai/gpt-5.2/pricing)._

![오픈라우터 Gemini 3.5 Flash Effective Pricing](/assets/img/posts/2026-07/or-gemflash-pricing.png)
_2026-07-19 보관 스냅샷. Gemini 3.5 Flash의 당시 가중 평균 입력은 $0.598/M이며 Vertex·AI Studio 행의 캐시힛은 66.5%·68.6%였다. 정가와의 차이에는 캐시뿐 아니라 공급자별 가격과 token share가 함께 들어간다. 원본 위치: [OpenRouter Gemini 3.5 Flash Pricing](https://openrouter.ai/google/gemini-3.5-flash/pricing)._

### 7월 23일 재확인 — 가격과 캐시 비율은 실제로 바뀌었다

같은 세 URL을 2026-07-23에 다시 열어 표의 현재값을 읽었다. 입력 가중 평균과 캐시 히트율이 모두 움직였다. 이 변화는 7월 19일 캡처가 잘못됐다는 뜻이 아니라, 롤링 집계의 관측 창과 공급자별 트래픽 구성이 바뀌었다는 뜻이다.

| 모델 | 7월 19일 입력 실효가 | 7월 23일 입력 실효가 | 공급자별 캐시 히트율 변화 |
|---|---:|---:|---|
| Claude Opus 4.8 | $1.67/M | **$1.83/M** | Anthropic 81.5→77.8%, Bedrock(US) 78.9→61.4%, Azure 0.0→86.5% |
| GPT-5.2 | $1.47/M | **$1.28/M** | OpenAI 14.2→29.0%, Azure 55.9→36.4% |
| Gemini 3.5 Flash | $0.598/M | **$0.754/M** | Vertex 66.5→51.2%, AI Studio 68.6→71.0% |

출력 가중 평균도 GPT-5.2는 $14.00→$14.02/M, Gemini 3.5 Flash는 $8.96→$8.95/M처럼 조금 움직였다. 공급자별 출력 단가와 최근 트래픽 비중이 섞인 결과다. Opus의 7월 23일 공급자 비중도 Claude Platform on AWS 38.7%·Bedrock(US) 27.1%·Vertex 24.3%·Anthropic 9.3%로 7월 19일과 달랐다. 이 비중은 표에 적힌 대로 최근 1일 값이며, 30일 실효가와 같은 시간창이 아니다.
...
_2026-07-19 보관 스냅샷. GLM-5.2 Providers 화면에서 같은 모델도 공급자별 Input/Output/Cache Read 단가, Latency, Throughput, uptime이 다르다. 초록색 할인값은 당시 공급자 프로모션이므로 고정 정가가 아니다. 원본 위치: [OpenRouter GLM-5.2 Providers](https://openrouter.ai/z-ai/glm-5.2/providers)._

7월 23일에는 같은 주소의 상단에 AkashML·StreamLake·Novita의 45% 할인, Alibaba의 41% 할인 등이 표시됐고 공급자 순서와 지연·처리량도 달라졌다. 예를 들어 당시 화면의 특정 할인율을 GLM-5.2의 정가나 장기 예상비용으로 쓰면 안 된다. 이 페이지는 **현재 구매 가능한 공급자 후보를 고르는 운영 화면**에 가깝다.
...
_2026-07-19 보관 스냅샷. Sonnet 4.6 카드의 47 tok/s는 공급자 중 가장 좋은 처리량 요약, 0.84s는 화면이 `p50, best provider`로 표시한 Latency다. 원본 위치: [OpenRouter Sonnet 4.6 Performance](https://openrouter.ai/anthropic/claude-sonnet-4.6/performance). 7월 23일 같은 카드는 55 tok/s·1.11s였다._
...
| 모델 | 실측 tok/s (25p 실효) | 오픈라우터 스루풋(7월 19일 캡처) | 오픈라우터 지연 <span class="term" data-tip="측정값의 50번째 백분위인 중앙값. 절반은 이 값 이하, 나머지 절반은 이 값 이상이며 산술평균이나 최악 지연을 뜻하지 않는다.">p50</span>(7월 19일) |
|---|---|---|---|
...
같은 성능 카드도 나흘 뒤 다음처럼 달라졌다. OpenRouter의 `best provider` 요약은 고정된 모델 고유 성능이 아니라 당시 공급자 측정의 중앙값 중 가장 좋은 값이다.

| 모델 | 7월 19일 캡처 | 7월 23일 현재 화면 | 변화 |
|---|---:|---:|---|
| Claude Sonnet 4.6 | 47 tok/s · 0.84s | **55 tok/s · 1.11s** | 처리량 증가, 지연 증가 |
| Claude Opus 4.8 | 63 tok/s · 0.72s | **78 tok/s · 1.06s** | 처리량 증가, 지연 증가 |
| GPT-5.2 | 52 tok/s · 2.42s | **47 tok/s · 1.41s** | 처리량 감소, 지연 감소 |
| Gemini 3.5 Flash | 135 tok/s · 1.66s | **134 tok/s · 1.59s** | 거의 비슷하지만 동일하지 않음 |
| GLM-5.2 | 171 tok/s · 0.36s | **119 tok/s · 0.45s** | 처리량 감소, 지연 증가 |

![오픈라우터 Opus 4.8 Performance](/assets/img/posts/2026-07/or-opus48-perf.png)
_2026-07-19 보관 스냅샷. Opus 4.8 카드의 63 tok/s·0.72s는 당시 `best provider` p50 요약이고, 아래 세 그래프는 공급자별 Throughput·Latency·E2E Latency의 1주 변화다. 원본 위치: [OpenRouter Opus 4.8 Performance](https://openrouter.ai/anthropic/claude-opus-4.8/performance). 7월 23일 같은 카드는 78 tok/s·1.06s였다._

![Artificial Analysis 모델 비교](/assets/img/posts/2026-07/aa-models-top.png)
_2026-07-19 보관 스냅샷. Artificial Analysis의 Intelligence·Output Speed·Latency·Price는 서로 다른 축이며 한 종합순위가 아니다. 기본 성능 방법론은 약 10K 입력, 최소 1.5K 출력의 고유 프롬프트를 약 3시간마다 측정하고 최근 72시간 p50을 표시한다. Output Speed는 첫 토큰 뒤의 생성 속도, Latency는 TTFT다. 원본 위치: [Artificial Analysis Models](https://artificialanalysis.ai/models), 방법: [Performance Benchmarking Methodology](https://artificialanalysis.ai/methodology/performance-benchmarking). 7월 19일 첫 화면의 Mercury 2는 769 t/s, North Mini Code는 0.32s였지만 7월 23일에는 각각 741 t/s·0.36s로 바뀌었다._

### 캡처 7장 재감사 — 과거 이미지와 현재 화면을 분리

| 이미지 | 7월 19일 이미지에 담긴 값 | 직접 원본 위치 | 7월 23일 재확인 결과 |
|---|---|---|---|
| `or-opus48-pricing.png` | Opus $1.67/$25.00, 공급자별 캐시·token share | [Opus Pricing](https://openrouter.ai/anthropic/claude-opus-4.8/pricing) | 현재 입력 실효가 $1.83. 이미지·캡션은 당시 값으로 일치 |
| `or-gpt52-pricing.png` | GPT-5.2 $1.47/$14.00, OpenAI·Azure 행 | [GPT-5.2 Pricing](https://openrouter.ai/openai/gpt-5.2/pricing) | 현재 입력 실효가 $1.28. 공급자별 캐시 비율도 변경 |
| `or-gemflash-pricing.png` | Gemini Flash $0.598/$8.96, Vertex·AI Studio 행 | [Gemini Flash Pricing](https://openrouter.ai/google/gemini-3.5-flash/pricing) | 현재 입력 실효가 $0.754. 이미지 출처·모델은 정확 |
| `or-glm52-providers.png` | GLM 공급자별 단가·지연·처리량 목록 | [GLM Providers](https://openrouter.ai/z-ai/glm-5.2/providers) | 현재 할인 공급자·할인율·정렬 순서 변경 |
| `or-sonnet46-perf.png` | Sonnet 47 tok/s·0.84s Performance 카드 | [Sonnet Performance](https://openrouter.ai/anthropic/claude-sonnet-4.6/performance) | 현재 55 tok/s·1.11s. 같은 화면의 롤링 값 |
| `or-opus48-perf.png` | Opus 63 tok/s·0.72s 및 1주 공급자 그래프 | [Opus Performance](https://openrouter.ai/anthropic/claude-opus-4.8/performance) | 현재 78 tok/s·1.06s. 그래프 구간도 앞으로 이동 |
| `aa-models-top.png` | Mercury 2 769 t/s, North Mini Code 0.32s | [AA Models](https://artificialanalysis.ai/models) | 현재 741 t/s·0.36s. 최근 72시간 p50이라 변동 |

7장 모두 **이미지 안의 수치와 7월 19일 캡션·모델·출처 URL은 맞았다.** 현재 화면과 다른 것은 잘못된 이미지를 붙였기 때문이 아니라 각 사이트의 롤링 통계가 갱신됐기 때문이다. 다만 OpenRouter 캡처는 모델명·주소가 픽셀 안에 없는 부분 캡처도 있어 이미지 단독으로 출처를 증명하지는 못한다. 이번에는 날짜·직접 URL·현재 재확인값을 함께 남겼다. 다음 캡처부터는 주소창과 모델명이 함께 보이는 전체 화면, 시간창·percentile 선택값, 같은 시각의 API 응답이나 원자료를 함께 보관한다.
...
[^orpricing]: [OpenRouter Claude Opus 4.8 Pricing](https://openrouter.ai/anthropic/claude-opus-4.8/pricing)의 Effective Pricing 설명은 고객이 prompt caching 후 실제 지불한 가격의 과거 30일 롤링 평균이라고 밝힌다. 공개 화면은 공급자별 Input/Output $/1M, Cache hit rate와 `Token share (1d)`를 함께 제공하지만 캐시 히트율의 정확한 분모와 가중 산식 전체는 설명하지 않는다. 가격과 token share의 시간창이 서로 다르다는 점도 주의해야 한다. 2026-07-23 재확인.
...
[^orperf]: [OpenRouter Provider Routing](https://openrouter.ai/docs/guides/routing/provider-selection)은 공급자별 latency·throughput percentile을 최근 5분 롤링 창으로 추적하며 p50·p75·p90·p99를 라우팅 조건에 사용할 수 있다고 설명한다. [Provider Integration](https://openrouter.ai/docs/guides/community/for-providers)은 throughput을 `output tokens ÷ generation time`으로 정의한다. Performance 화면은 Latency를 round-trip이라고 쓰지만 [FAQ](https://openrouter.ai/docs/faq)는 모델 페이지 latency를 TTFT라고 설명해 정의가 일치하지 않는다. 2026-07-23 확인.
[^aa]: [Artificial Analysis — Models](https://artificialanalysis.ai/models)와 [Performance Benchmarking Methodology](https://artificialanalysis.ai/methodology/performance-benchmarking) (2026-07-23 재확인). 기본 10K 입력 workload를 하루 8회 측정하고 최근 72시간 p50을 표시한다. Output Speed는 첫 토큰 뒤의 생성 속도, TTFT는 요청부터 첫 토큰까지다. OpenRouter와 별도 사업자의 측정이라 교차 참고에는 유용하지만 우리 25페이지 프롬프트의 재현 실험은 아니다.
