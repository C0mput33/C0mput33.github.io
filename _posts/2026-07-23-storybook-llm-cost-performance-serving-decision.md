...
가장 싼 행과 가장 비싼 행은 약 240배 차이지만, 이것은 200단어 안팎의 구형 프롬프트에 한정된다. Qwen3.6과 Qwen3.5의 비용이 단어 수에 비해 높은 이유는 보이지 않는 추론 토큰이 많았기 때문이다. 그래서 API 비교는 토큰 정가만 보지 않고 **유효 결과 한 편을 얻는 실현 원가**로 다시 확인해야 한다.
