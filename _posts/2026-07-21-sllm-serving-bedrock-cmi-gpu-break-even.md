...
평가 파이프라인이 만든 부산물 중에 <span class="term" data-tip="Direct Preference Optimization. 어느 쪽이 더 나은가라는 선호쌍으로 모델을 직접 최적화하는 학습법. 별도 보상 모델과 강화학습 없이 선호 데이터만으로 정렬을 수행한다. 평가 리그의 판정 기록이 그대로 이 학습 데이터가 된다.">DPO</span> 선호쌍 746개가 있다.[^dpo] 심판단이 "이 동화가 더 낫다"고 판정한 기록을 <span class="term" data-tip="Hugging Face의 정렬 학습 라이브러리(Transformer Reinforcement Learning). SFT·DPO·PPO 트레이너를 제공하며, 선호쌍 데이터 형식의 사실상 표준이다.">TRL</span> 호환 형식으로 내보낸 것인데, 이걸로 Qwen3-4B를 파인튜닝하는 것이 다음 단계다. 학습은 아직 하지 않았다. 그 전에 먼저 답해야 할 질문이 있었다. 파인튜닝한 모델은 어디에 올리나.

후보는 넷이다. 기성 모델을 <span class="term" data-tip="모델 가중치와 GPU를 직접 들지 않고, 사업자가 호스팅하는 모델을 토큰 단위로 사서 쓰는 방식. 고정비가 0이라 저물량에 유리하지만 내 가중치를 올릴 수 없고, 모델의 수명과 동작이 사업자 손에 있다.">관리형 API</span>로 계속 쓰기, 자체 가중치를 Bedrock Custom Model Import(CMI)로 올리기, EC2 GPU에 <span class="term" data-tip="오픈소스 LLM 서빙 엔진. KV 캐시를 페이지 단위로 관리하는 PagedAttention으로 같은 GPU에서 더 많은 동시 요청을 처리한다. 자체 GPU 서빙의 사실상 표준이다.">vLLM</span>으로 직접 서빙하기, 로컬 맥북. 각각의 단가를 확인하고 물량 구간별로 계산해 보니 결론은 명확했다. 이 글은 그 계산 과정이다.
...
| | 자체 가중치 | 고정비 | 권당 비용 | <span class="term" data-tip="유휴 상태로 잠들어 있던 모델 서버가 첫 요청을 받고 깨어나 가중치를 메모리에 올리는 지연. 상시 켜두면 없앨 수 있지만 그만큼 고정비를 낸다. 비동기 작업 화면 뒤에 숨기면 사용자는 못 느낀다.">콜드스타트</span> |
|---|---|---|---|---|
...
교체 여부를 정하는 것은 감이 아니라 이미 만들어 둔 평가 파이프라인이다. CMI 엔드포인트를 [BT 리그](/posts/llm-eval-pipeline-from-scratch-bradley-terry/)에 후보로 넣고, 기성 모델과 <span class="term" data-tip="진짜 값이 이 범위 안에 있을 것이라고 정해진 신뢰 수준(예: 95%)으로 말할 수 있는 구간. 같은 실험을 100번 반복하면 그중 95번은 구간이 진짜 값을 포함한다는 뜻이다. 두 후보의 구간이 겹치면 우열을 통계적으로 단정할 수 없다.">신뢰구간</span>이 겹치는지로 판정한다. 서빙 계산과 평가 시스템이 여기서 연결된다.
...
2. 가중치는 <span class="term" data-tip="Hugging Face의 모델 로딩·학습 표준 라이브러리. 모델이 요구하는 최소 버전이 config에 기록되며, 서빙 플랫폼이 지원하는 버전보다 새 모델은 로드가 거부된다.">transformers</span> 4.51.3 기준으로 저장.
...
M5 Pro 48GB는 서빙 후보가 아니라 실험대다. 평가 파이프라인의 로컬 모드는 <span class="term" data-tip="오픈 웨이트 모델을 내 컴퓨터에서 직접 돌리게 해주는 로컬 LLM 실행기. API 비용이 0이라 온디바이스 후보 선별에 쓴다.">Ollama</span>로 qwen3 4B~0.6B급 후보 7개를 돌리는 설정이 이미 있고,[^local] 35B-A3B <span class="term" data-tip="llama.cpp 계열이 쓰는 단일 파일 모델 포맷. 양자화된 가중치와 실행에 필요한 메타데이터를 한 파일에 담아, 내려받아 바로 실행되게 한다.">GGUF</span>도 Q4~Q6로 적재된다. 파인튜닝 전후 비교 같은 반복 실험을 토큰 비용 없이 돌리는 용도로 로컬을 쓰고, 서빙 판단은 위 계산대로 클라우드에서 한다.
