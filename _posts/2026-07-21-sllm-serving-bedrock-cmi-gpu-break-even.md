...
## 30시간·90시간과 API 네 종을 같은 기준으로 비교하기

시간만으로는 GPU와 API를 비교할 수 없다. GPU는 켜 둔 시간에 과금되고 API는 입력·출력·캐시 토큰에 과금되므로, GPU가 그 시간 안에 **같은 품질의 성공 결과를 몇 편 만드는지**가 필요하다. 아직 Qwen3.6을 <span class="term" data-tip="NVIDIA의 서빙·그래픽 겸용 GPU로 VRAM 48GB. 모델 가중치와 KV 캐시가 VRAM에 다 들어가야 서빙이 성립하므로, 24GB(L4)냐 48GB(L40S)냐가 올릴 수 있는 모델의 상한을 가른다.">L40S</span>에서 실측하지 않았으므로 여기서는 비용이 같아지기 위해 필요한 처리량을 역산한다. 실제 성능처럼 쓰지 않는다.

### API 비교값은 같은 25페이지 실측에서 가져온다

2026년 7월 19일 같은 25페이지 생성 프롬프트를 모델마다 5회 호출했다. Claude·GPT·GLM·Kimi 모두 본문이 비어 있지 않은 응답은 5/5였지만, `childlit-v3` 사람·LLM 품질 게이트를 통과했다는 뜻은 아니다. 아래 `OpenRouter 실청구/편`은 응답의 `usage.cost`를 5회 평균한 관측값이다.[^api-four-run]

| 비교 모델 | 공식 직접 API 정가 입력/캐시/출력 ($/MTok)[^api-list-price] | OpenRouter 실청구/편 | 같은 토큰을 직접 API 정가로 재계산[^api-replay] | 비어 있지 않은 응답 | 실측 캐시 히트 |
|---|---:|---:|---:|---:|---:|
...
GPT는 최신 계열 전체를 뜻하지 않는다. 동일 프롬프트 실측이 있는 **GPT-5.2**만 비교한다. OpenAI는 현재 GPT-5.2를 이전 프론티어 모델로 분류하고 GPT-5.6을 권장하지만, GPT-5.6으로 이 5회 실측을 하지 않았으므로 가격표만으로 결과당 비용을 지어내지 않는다.[^api-list-price]

GLM과 Kimi의 OpenRouter 실청구가 공식 직접 API 재계산보다 낮은 이유는 당시 더 싼 제3자 공급자에도 라우팅됐기 때문이다. GLM은 Baidu·Inceptron·Ambient·WandB, Kimi는 Inceptron·DeepInfra·Baidu·StreamLake가 섞였다. 따라서 직접 API를 쓸 계획이면 오른쪽 재계산값을 예산 초안으로 쓰고 새로 실측해야 한다. OpenRouter를 계속 써도 공급자와 단가는 바뀔 수 있으므로 공급자 고정 또는 가격 상한이 필요하다.

### EC2를 월 30시간·90시간 켰을 때

서울 `g6e.xlarge` 온디맨드 $2.288/시간과 100GB gp3 $9.12/월을 사용한다. 30시간은 하루 평균 약 1시간, 90시간은 약 3시간을 실행한 예시다. 시작·모델 적재·생성·유휴 종료가 모두 실행 시간에 들어간다.[^qwen36-eight-hours]

| 월 누적 `running` 시간 | EC2 컴퓨트 | 100GB gp3 | 단순 합계 |
|---:|---:|---:|---:|
...
공인 IPv4는 30시간에 $0.15, 90시간에 $0.45가 추가될 수 있다. 표에는 세금·CloudWatch·스냅샷·제어 API·데이터 전송·실패 재시도·운영 인건비를 넣지 않았다. 그래서 실제 청구액의 하한에 가깝다.

### 월 생성량을 정해 놓고 보면

아래 API 금액은 같은 실측의 `OpenRouter 실청구/편 × 월 결과 수`다. EC2 금액은 생성량과 무관하게 각각 30시간·90시간을 모두 사용했다고 가정한 고정비다. 단, 자체 Qwen이 그 수량을 해당 실행 시간 안에 끝내고 같은 품질 기준을 통과할 때만 유효한 비교다.

| 월 결과 수 | Claude Opus 4.8 (OR) | GPT-5.2 (OR) | GLM-5.2 (OR) | Kimi K2.6 (OR) |
|---:|---:|---:|---:|---:|
...
| 자체 Qwen 경로 | 생성량과 무관한 EC2+EBS 합계 | 비교가 성립하는 조건 |
|---|---:|---|
| EC2 30시간 | $77.76 | 위 수량을 30시간 안에 완료 |
| EC2 90시간 | $215.04 | 위 수량을 90시간 안에 완료 |

이 표는 품질 순위표가 아니다. 예를 들어 1,000편에서 EC2 30시간이 Claude보다 $7.83 비싸지만, 3,000편에서는 $132.04 싸다. 그 차이를 실제 절감액으로 인정하려면 Qwen이 30시간 안에 3,000편, 즉 시작과 적재를 포함해 평균 36초마다 한 편을 완료하고 Claude와 사전에 정한 품질 허용폭도 지켜야 한다. 현재는 그 처리량을 아직 측정하지 않았다.

### API와 비용이 같아지려면 몇 편을 만들어야 하나

다음은 EC2+EBS 합계를 위 OpenRouter 실청구 평균으로 나눈 값이다. `편/시간`은 그 수량을 각각 30시간·90시간 안에 완료해야 하는 평균 처리량이고, `초/편`에는 순수 생성뿐 아니라 시작·모델 적재·유휴 시간의 몫도 들어간다. 작은 화면에서도 수치를 한 번에 읽을 수 있도록 30시간과 90시간을 나눴다.

**EC2를 월 30시간 사용하는 경우**

| 대체 API | 손익분기 결과 수 | 필요한 평균 | 허용 실행 초/편 |
|---|---:|---:|---:|
...
30시간과 90시간은 할인 상품 두 개가 아니다. 같은 동화 수라면 30시간 경로가 항상 싸고, 90시간은 처리량이 부족해 더 오래 켜야 할 때 드는 비용이다. 90시간에서 편당 허용 시간이 조금 늘어나는 것은 EBS $9.12가 더 많은 실행 시간에 분산되기 때문이다.

현재 결론은 분명하다.

- 월 수백~1천 편에서는 네 API가 요청 기반 G6e보다 싸다. 특히 GLM의 당시 OpenRouter 실청구액은 격차가 크다.
- 자체 Qwen이 Claude를 비용으로 이기려면 30시간 구성에서 시작·적재·실패까지 포함해 평균 97초 이내로 같은 품질의 결과를 내야 한다.
- GLM을 이기려면 평균 12.6초 이내가 필요하다. 공식 <span class="term" data-tip="8비트 부동소수점 형식 계열. 가중치와 활성값을 더 작게 만들 수 있지만 하드웨어·커널 지원과 보정 방식에 따라 정확도와 실제 메모리 절감 폭이 달라진다.">FP8</span> 35B-A3B 한 장 서빙에서 아직 확인하지 못한 매우 높은 통과선이다.
- 비용만 같아도 교체하지 않는다. 새 `childlit-v3` 평가의 품질 허용폭, 성공률, 지연과 총원가를 모두 통과해야 한다.
...
여기서 구분해야 할 것이 있다. vLLM은 모델을 효율적으로 실행하고 <span class="term" data-tip="OpenAI Chat Completions와 비슷한 URL·JSON 요청·응답 형식을 구현해 같은 클라이언트 코드로 다른 모델 서버를 호출할 수 있게 한 인터페이스. 세부 지원 필드는 서버마다 달라 별도 확인이 필요하다.">OpenAI 호환 API</span>와 메트릭을 제공하는 추론 엔진이다. 인스턴스를 만들고 없애는 오토스케일링 제어면 전체는 아니다. EC2 한 대에서 vLLM을 실행했다고 요청이 없을 때 인스턴스가 저절로 꺼지는 것도 아니다. <span class="term" data-tip="유휴 시간에는 실행 인스턴스를 0개로 줄였다가 새 요청이 오면 다시 시작하는 운영 방식. 유휴 비용을 줄이는 대신 첫 요청에는 컨테이너 시작과 모델 적재 지연이 붙는다.">스케일 제로</span>가 필요하면 CMI처럼 공급자가 수명주기를 관리하는 제품을 쓰거나, 별도의 서빙 플랫폼과 노드 오토스케일러를 붙여야 한다.
...
[^api-four-run]: 2026-07-19 `eval/analysis/cost-per-book/results_20260719T094911Z.json`의 동일 25페이지 프롬프트 5회 원자료를 다시 계산했다. 모델별 본문 문자 수가 0보다 큰 응답 수, `usage.cost`, 입력·캐시 입력·출력 토큰을 사용했다. 당시 실제 공급자는 GPT는 OpenAI, Claude는 Anthropic이었고 GLM은 Baidu·Inceptron·Ambient·WandB, Kimi는 Inceptron·DeepInfra·Baidu·StreamLake가 섞였다. OpenRouter의 [`usage.cost`](https://openrouter.ai/docs/cookbook/administration/usage-accounting)는 해당 요청에 실제 차감된 비용이다. 2026-07-23의 [Models API](https://openrouter.ai/api/v1/models) 단가는 이미 일부 달라졌으므로 과거 실청구액과 현재 정가를 섞지 않았다.
[^api-list-price]: 2026-07-23 조회한 공식 직접 API 정가다. [Anthropic Claude Opus 4.8](https://www.anthropic.com/claude/opus)은 입력·캐시 읽기·출력 $5/$0.50/$25, [OpenAI GPT-5.2](https://developers.openai.com/api/docs/models/gpt-5.2)는 $1.75/$0.175/$14, [Z.ai GLM-5.2](https://docs.z.ai/guides/overview/pricing)는 $1.40/$0.26/$4.40, [Moonshot Kimi K2.6](https://platform.kimi.ai/docs/pricing/chat-k26)은 $0.95/$0.16/$4.00 per MTok이다. OpenAI 문서는 GPT-5.2를 이전 프론티어 모델로 표시하고 GPT-5.6 사용을 권한다.
[^api-replay]: 직접 API 정가 재계산은 모델별 5회 평균에 `(입력-캐시 입력)×입력 단가 + 캐시 입력×캐시 읽기 단가 + 출력×출력 단가`를 적용했다. 실제 직접 API 청구 관측이 아니라 같은 토큰 수를 가정한 산술 추정이며, 캐시 생성·보관, 공급자별 토큰 집계 차이, 도구 호출, 세금과 재시도는 포함하지 않는다.
