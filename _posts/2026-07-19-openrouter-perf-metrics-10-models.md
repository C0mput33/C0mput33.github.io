...
- **Cache hit rate 열** — 공급자별 캐시 히트율. 같은 모델이라도 어느 <span class="term" data-tip="들어온 요청을 여러 서버·모델·공급자 후보 중 하나로 보내는 선택 과정. 가용성, 현재 부하, 비용, 캐시 재사용 가능성처럼 목적에 맞는 기준과 실패 시 대체 경로가 필요하다.">라우팅</span> 공급자에 걸리느냐에 따라 0%에서 80%대까지 벌어진다.
