...
description: >-
  ...
  수집해 내 실측값과 전수 대조했다. 7월 19·21일 캡처와 7월 23일 현재값을 분리하고, 변동된 공개 지표로
  운영 참고순위도 다시 계산했다. 실측 토큰으로 환산한 분당 토큰(OTPM) 표도 함께 설명한다.
...
> **먼저 읽기 — 캡처와 현재 화면은 같은 값이 아니다**
> 이 글의 Performance 캡처 3장은 2026-07-19, Effective Pricing 캡처 6장은 2026-07-21의 보관 스냅샷이다. OpenRouter의 공개 지표는 공급자 상태·캐시 사용·트래픽 구성에 따라 계속 갱신된다. Effective Pricing은 최근 30일 롤링 평균, Token share는 최근 1일 비중이며, 공급자 라우팅 통계는 최근 5분 창을 사용한다.[^oreffective][^routing] 따라서 같은 주소를 다시 열면 수치와 공급자 순서가 달라지는 것이 정상이다. 아래 원래 표는 당시 관측값으로 보존하고, **2026-07-23 08:43 KST에 10개 모델을 다시 읽은 값**을 별도 표로 병기했다.
...
| 모델 | 실측 tok/s | 실측 권당 시간 | OR 스루풋(07-19) | OR 지연 p50(07-19) | OR 실효 입력가(07-19 대표가) |
|---|---|---|---|---|---|
...
### 7월 23일 재확인 — 10개 모델 모두 값이 움직였다

같은 10개 Performance URL을 2026-07-23 08:43 KST에 다시 열어 카드의 `P50, best provider`와 Effective Pricing의 가중 입력가를 읽었다. 아래 `07-19` 열은 위 보관 표, `07-23` 열은 재확인 시각의 화면값이다. 몇 분 뒤에도 반올림 끝자리가 바뀔 수 있으므로 이 표 역시 날짜·시각이 붙은 스냅샷이다.

| 모델 | OR 처리량 07-19→07-23 | OR 지연 07-19→07-23 | 입력 실효가 07-19→07-23 |
|---|---:|---:|---:|
| gemini-3.1-flash-lite | 94→**105 tok/s** | 0.59→**0.56s** | $0.177→**$0.175/M** |
| claude-haiku-4.5 | 109→**92 tok/s** | 0.26→**0.37s** | $0.565→**$0.666/M** |
| gemini-3.5-flash | 135→**134 tok/s** | 1.66→**1.59s** | $0.598→**$0.754/M** |
| claude-opus-4.8 | 63→**78 tok/s** | 0.72→**1.06s** | $1.67→**$1.82/M** |
| gemini-3.1-pro | 110→**101 tok/s** | 2.89→**2.79s** | $1.53→**$1.29/M** |
| glm-5.2 | 171→**119 tok/s** | 0.36→**0.45s** | $0.206→**$0.347/M** |
| gpt-5.2 | 52→**47 tok/s** | 2.42→**1.41s** | $1.47→**$1.28/M** |
| gemma-4-31b | 133→**87 tok/s** | 0.31→**0.32s** | $0.168→**$0.149/M** |
| qwen3.6-35b-a3b | 162→**152 tok/s** | 0.25→**0.22s** | $0.099→**$0.144/M** |
| kimi-k2.6 | 194→**144 tok/s** | 0.17→**0.24s** | $0.353→**$0.432/M** |

방향도 모델마다 달랐다. Flash Lite의 처리량은 올랐지만 Kimi·GLM·Haiku는 내려갔고, GPT-5.2는 처리량이 낮아지는 동안 표시 지연도 낮아졌다. Opus 입력 실효가는 같은 날 앞선 [캐시편 재확인](/posts/cache-hit-measured-vs-benchmark-sites/)에서는 $1.83/M, 이 글을 읽은 08:43에는 $1.82/M로 표시됐다. 즉 `현재값`도 영구 상수가 아니라 조회 시각의 관측값이다.
...
_2026-07-19 보관 스냅샷. Kimi K2.6의 194 tok/s와 지연 0.17s는 당시 공급자 중 가장 좋은 요약값이고, 내 실측 372.5초와 측정 정의가 다르다. 원본 위치: [OpenRouter Kimi K2.6 모델 페이지](https://openrouter.ai/moonshotai/kimi-k2.6/performance). 2026-07-23 08:43 KST에는 144 tok/s·0.24s였다._
...
_2026-07-19 보관 스냅샷. Gemini 3.1 Flash Lite의 당시 카드값 94 tok/s·0.59s와 내 실측 263 tok/s·6.9초는 서로 다른 모집단의 값이다. 원본 위치: [OpenRouter Gemini 3.1 Flash Lite 모델 페이지](https://openrouter.ai/google/gemini-3.1-flash-lite/performance). 2026-07-23 08:43 KST에는 105 tok/s·0.56s였다._

![오픈라우터 Gemini 3.5 Flash Performance](/assets/img/posts/2026-07/or-gemflash-perf.png)
_2026-07-19 보관 스냅샷. Gemini 3.5 Flash의 당시 카드 요약은 135 tok/s·1.66s였고, 아래 공급자별 주간 평균도 서로 달랐다. 오류율·캐시 히트 그래프는 **공급자 운영 진단**이지 모델 품질 점수가 아니다. 원본 위치: [OpenRouter Gemini 3.5 Flash 모델 페이지](https://openrouter.ai/google/gemini-3.5-flash/performance). 2026-07-23 08:43 KST에는 134 tok/s·1.59s였다._
...
_2026-07-21 보관 스냅샷. Claude Haiku 4.5의 대표가는 $1/$5, 당시 30일 가중 평균은 $0.680/$5.00이었다. Anthropic 47.1%·Vertex 54.4%·Vertex(EU) 76.3%는 공급자별 캐시 히트율이고, 73.0%·18.2%·7.6%는 최근 1일 token share다. 원본 위치: [OpenRouter Claude Haiku 4.5 모델 페이지](https://openrouter.ai/anthropic/claude-haiku-4.5/pricing). 2026-07-23 08:43 KST 가중 평균은 $0.666/$5.00이었다._
...
_2026-07-21 보관 스냅샷. Gemini 3.1 Pro는 당시 $1.43/$12.05, Vertex 캐시 37.1%·token share 77.1%, AI Studio 캐시 55.6%·token share 22.9%였다. 원본 위치: [OpenRouter Gemini 3.1 Pro Preview 모델 페이지](https://openrouter.ai/google/gemini-3.1-pro-preview/pricing). 2026-07-23 08:43 KST 가중 평균은 $1.29/$12.02였다._

![오픈라우터 Qwen3.6-35B-A3B Effective Pricing](/assets/img/posts/2026-07/or-qwen36-pricing.png)
_2026-07-21 보관 스냅샷. Qwen3.6-35B-A3B는 당시 $0.143/$1.07이었다. AkashML이 token share 55.2%라 캐시 히트가 높은 Parasail 63.9%·Weights & Biases 65.2%만 보고 전체 절감률을 추정하면 안 된다. 원본 위치: [OpenRouter Qwen3.6-35B-A3B 모델 페이지](https://openrouter.ai/qwen/qwen3.6-35b-a3b/pricing). 2026-07-23 08:43 KST 가중 평균은 $0.144/$1.09였다._

![오픈라우터 Gemma 4 31B Effective Pricing](/assets/img/posts/2026-07/or-gemma-pricing.png)
_2026-07-21 보관 스냅샷. Gemma 4 31B는 당시 $0.150/$0.416이었다. 보이는 행만 해도 여러 공급자의 단가·캐시 히트·token share가 크게 다르다. 원본 위치: [OpenRouter Gemma 4 31B 모델 페이지](https://openrouter.ai/google/gemma-4-31b-it/pricing). 2026-07-23 08:43 KST 가중 평균은 $0.149/$0.425였다._

![오픈라우터 Gemini 3.1 Flash Lite Effective Pricing](/assets/img/posts/2026-07/or-flashlite-pricing.png)
_2026-07-21 보관 스냅샷. Gemini 3.1 Flash Lite는 당시 $0.175/$1.46, AI Studio와 Vertex의 token share가 53.0%·47.0%로 거의 반반이었다. 원본 위치: [OpenRouter Gemini 3.1 Flash Lite 모델 페이지](https://openrouter.ai/google/gemini-3.1-flash-lite/pricing). 2026-07-23 08:43 KST 가중 평균은 $0.175/$1.48이었다._

![오픈라우터 Kimi K2.6 Effective Pricing](/assets/img/posts/2026-07/or-kimi-pricing.png)
_2026-07-21 보관 스냅샷. Kimi K2.6은 당시 $0.373/$3.61이었다. SiliconFlow의 캐시 히트 84.7%와 token share 34.7%가 보이지만, 다른 공급자 행까지 합친 30일 평균이 맨 위 값이다. 원본 위치: [OpenRouter Kimi K2.6 모델 페이지](https://openrouter.ai/moonshotai/kimi-k2.6/pricing). 2026-07-23 08:43 KST 가중 평균은 $0.432/$3.69였다._
...
7월 21일 캡처와 7월 23일 재확인값만 따로 보면 다음과 같다.

| 모델 | 07-21 가중 in/out | 07-23 08:43 KST 가중 in/out | 입력가 변화 |
|---|---:|---:|---:|
| gemini-3.1-flash-lite | $0.175 / $1.46 | **$0.175 / $1.48** | 0.0% |
| gemma-4-31b | $0.150 / $0.416 | **$0.149 / $0.425** | -0.7% |
| qwen3.6-35b-a3b | $0.143 / $1.07 | **$0.144 / $1.09** | +0.7% |
| gemini-3.1-pro | $1.43 / $12.05 | **$1.29 / $12.02** | -9.8% |
| claude-haiku-4.5 | $0.680 / $5.00 | **$0.666 / $5.00** | -2.1% |
| kimi-k2.6 | $0.373 / $3.61 | **$0.432 / $3.69** | +15.8% |

세 시점을 비교하면 gemma의 페이지 대표 입력 단가 표시는 $0.22에서 $0.10으로 바뀌었고, qwen·haiku의 30일 가중 입력가도 한 방향으로 고정되지 않았다. 다만 이것은 동일 공급자·동일 트래픽을 고정한 가격 실험이 아니다. 공급자 목록, 각 단가, token share, 캐시 사용이 모두 움직일 수 있어 어느 한 요인을 원인으로 단정하지 않았다. 공개값을 운영 예산의 고정 상수로 쓰지 말고, 실제 청구액과 함께 날짜·시각을 붙여 저장해야 한다.
...
| 이미지 | 보관 이미지에 담긴 값 | 직접 원본 위치 | 2026-07-23 08:43 KST 재확인 |
|---|---|---|---|
| `or-kimi-perf.png` | Kimi 194 tok/s·0.17s Performance | [Kimi Performance](https://openrouter.ai/moonshotai/kimi-k2.6/performance) | 현재 144 tok/s·0.24s. 과거 이미지·캡션은 일치 |
| `or-flashlite-perf.png` | Flash Lite 94 tok/s·0.59s Performance | [Flash Lite Performance](https://openrouter.ai/google/gemini-3.1-flash-lite/performance) | 현재 105 tok/s·0.56s |
| `or-gemflash-perf.png` | Gemini Flash 135 tok/s·1.66s 및 공급자 그래프 | [Gemini Flash Performance](https://openrouter.ai/google/gemini-3.5-flash/performance) | 현재 134 tok/s·1.59s. 그래프 기간도 앞으로 이동 |
| `or-haiku-pricing.png` | Haiku $0.680/$5.00 Effective Pricing | [Haiku Pricing](https://openrouter.ai/anthropic/claude-haiku-4.5/pricing) | 현재 $0.666/$5.00 |
| `or-gempro-pricing.png` | Gemini Pro $1.43/$12.05 | [Gemini Pro Pricing](https://openrouter.ai/google/gemini-3.1-pro-preview/pricing) | 현재 $1.29/$12.02 |
| `or-qwen36-pricing.png` | Qwen $0.143/$1.07 | [Qwen Pricing](https://openrouter.ai/qwen/qwen3.6-35b-a3b/pricing) | 현재 $0.144/$1.09 |
| `or-gemma-pricing.png` | Gemma $0.150/$0.416 | [Gemma Pricing](https://openrouter.ai/google/gemma-4-31b-it/pricing) | 현재 $0.149/$0.425 |
| `or-flashlite-pricing.png` | Flash Lite $0.175/$1.46 | [Flash Lite Pricing](https://openrouter.ai/google/gemini-3.1-flash-lite/pricing) | 현재 $0.175/$1.48 |
| `or-kimi-pricing.png` | Kimi $0.373/$3.61 | [Kimi Pricing](https://openrouter.ai/moonshotai/kimi-k2.6/pricing) | 현재 $0.432/$3.69 |

9장 모두 이미지 안의 값과 캡처 당시 캡션·모델·출처 URL은 맞았다. 현재 화면과 다른 것은 잘못된 이미지를 붙였기 때문이 아니라 롤링 집계가 갱신됐기 때문이다. 한계도 있다. 캡처가 섹션만 잘라져 있어 이미지 픽셀 안에는 모델명·주소가 없다. 따라서 과거 캡처의 출처 동일성을 제3자가 이미지 하나만으로 증명할 수는 없다. 이 글에서는 파일명, 캡처 날짜, 보이는 수치, 직접 URL과 현재 재확인값을 함께 남겼다. 다음 수집부터는 페이지 제목·주소·조회 시각·percentile·그래프 기간이 보이는 전체 캡처 또는 같은 시각의 API 응답을 함께 보관한다.

## 7월 19일 운영 참고순위 — 현재 순위가 아니다
...
### 7월 23일 공개 지표로 같은 순위합을 다시 계산하면

실측 시간·실청구액은 7월 19일 원자료를 그대로 두고, 변동되는 OR 처리량·지연 두 열만 7월 23일 08:43 KST 값으로 교체했다. 그러므로 이것도 동일 시점의 완전한 재실험이 아니라 **공개 운영 참고선의 민감도 점검**이다.

| 참고순위 | 모델 | 시간 순위 | 실측 비용 순위 | OR 처리량 순위(07-23) | OR 지연 순위(07-23) | 순위합 |
|---|---|---:|---:|---:|---:|---:|
| 1 | gemini-3.1-flash-lite | 1 | 2 | 4 | 5 | **12** |
| 2 | claude-haiku-4.5 | 2 | 4 | 6 | 3 | **15** |
| 3= | glm-5.2 | 6 | 3 | 3 | 4 | **16** |
| 3= | kimi-k2.6 | 9 | 5 | 1 | 1 | **16** |
| 5 | gemma-4-31b | 8 | 1 | 7 | 2 | **18** |
| 6 | gemini-3.5-flash | 3 | 6 | 2 | 8 | **19** |
| 7= | claude-opus-4.8 | 4 | 9 | 8 | 6 | **27** |
| 7= | gemini-3.1-pro | 5 | 8 | 5 | 9 | **27** |
| 9 | gpt-5.2 | 7 | 7 | 9 | 7 | **30** |
| 제외 | qwen3.6-35b-a3b | — | — | — | — | 3/5 유효 응답 |

공개 지표 두 열만 갱신했는데 Flash Lite가 공동 2위에서 1위로, Haiku는 1위에서 2위로 바뀌었다. 이 변화 자체가 순위합을 모델의 고정된 실력 순위로 보면 안 되는 이유다. 품질·안전·실패율을 포함하지 않은 동일 가중 순위합은 후보를 좁히는 참고값일 뿐이다.
...
공급자 운영 지표는 <span class="term" data-tip="여러 회사의 LLM을 하나의 API와 결제로 호출하게 해주는 중계 서비스. 모델마다 계정을 따로 만들 필요가 없어 다모델 비교 실험에 편하다.">OpenRouter</span>, 모델 중립 비교는 Artificial Analysis, 품질 선호는 LMArena에서 확인할 수 있다. 이 셋은 서로 대신할 수 없다. Kimi의 7월 19일 공개 처리량 194 tok/s, 7월 23일 144 tok/s와 이 워크로드의 372.5초처럼 공개 운영 지표는 움직이고 실제 완료 시간과도 반대로 움직일 수 있다. 단순 순위합 1위도 창작 품질 1위를 뜻하지 않는다. 위 캐파시티 표에서 glm-5.2가 동화 10권을 동시에 처리할 때의 관측 요구량은 분당 약 4.4만 출력 토큰이다. 실제 용량은 여기에 재시도·피크·p95 여유를 더하고 새 부하 시험으로 확정한다.
...
[^oreffective]: [OpenRouter Claude Haiku 4.5 Pricing](https://openrouter.ai/anthropic/claude-haiku-4.5/pricing)은 Effective Pricing을 prompt caching 이후 고객이 실제로 지불한 과거 30일 롤링 평균으로 설명하고 provider별 Input/Output $/1M, Cache hit rate, `Token share (1d)`를 공개한다. 헤드라인 대표가 $1/$5도 같은 페이지에서 확인했다. 2026-07-23 재확인.
